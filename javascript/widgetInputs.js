...
const VALID_TYPES = ["STRING", "combo", "number", "AV_TASK_ID"];

ComfyWidgets["AV_TASK_ID"] = (node, inputName, inputData, app) => {
...
app.extensions = app.extensions.filter((ext) => ext.name !== "Comfy.WidgetInputs")
	...
	registerCustomNodes() {
		class PrimitiveNode {
			...
			#createWidget(inputData, node, widgetName) {
				...
				if (widget.type === "number" && widgetName == 'seed') {
					addValueControlWidget(this, widget, "fixed");
				}
				this.addWidget("text", "param", widgetName, () => { }, {})
			...
			#onLastDisconnect() {
				...
				this.outputs[0].type = "*";
				this.outputs[0].name = "Connect to AV Input";
		...
		PrimitiveNode.category = "ArtVenture";
	},
});
