...
class AVControlNetEfficientLoader(ControlNetApply):
    ...
    preprocessors = ["None"] + list(control_net_preprocessors.keys())
    ...
    def load_controlnet(
        ...
        if control_net_override != "None":
            if control_net_override not in self.controlnets:
                print(
                    f"Warning: Not found ControlNet model {control_net_override}. Use {control_net_name} instead."
                )
            else:
        ...
        if preprocessor != "None":
            ...
            preprocessor_args = {
                ...
                for i, key in enumerate(
                    preprocessor_class.INPUT_TYPES()["required"].keys()
                )
            }
